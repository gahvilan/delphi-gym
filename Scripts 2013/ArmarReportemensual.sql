...
BEGIN
  ...
  SALDO = CANT;
  SUSPEND;
  /*BUSCAR TODOS LOS INGRESOS POR CROSSFIT*/
  FOR SELECT CLIENTE, DISCIPLINA from PAGO_CROSSFIT where (EXTRACT(MONTH FROM FECHA_EFECTIVO)=:MES AND EXTRACT(YEAR FROM FECHA_EFECTIVO)=:ANIO)
      INTO :CLI, :DIS
  DO
  BEGIN
       SELECT NOMBRE||' '||APELLIDO FROM CLIENTES WHERE (ID_CLIENTE=:CLI) INTO :INGRESO;
