inherited FFechaPago: TFFechaPago
  Left = 363
  Top = 131
  VertScrollBar.Range = 0
  BorderStyle = bsSingle
  Caption = 'FFechaPago'
  ClientWidth = 683
  Font.Name = 'Arial'
  OldCreateOrder = True
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 14
  inherited PEncabezado: TPanel
    Width = 683
    inherited Image1: TImage
      Width = 675
      Stretch = True
    end
    inherited LTitulo: TLabel
      Width = 151
      Caption = 'Fecha de Pago'
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 177
    Width = 683
    Height = 201
    Align = alClient
    TabOrder = 1
    object SGDatos: TStringGrid
      Left = 1
      Top = 1
      Width = 681
      Height = 199
      Align = alClient
      ColCount = 3
      FixedCols = 0
      RowCount = 2
      Options = [goFixedVertLine, goFixedHorzLine, goHorzLine, goRangeSelect, goRowSelect]
      TabOrder = 0
      ColWidths = (
        205
        204
        262)
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 97
    Width = 683
    Height = 80
    Align = alTop
    Color = clWhite
    TabOrder = 2
    object GBCliente: TGroupBox
      Left = 441
      Top = 1
      Width = 241
      Height = 78
      Align = alClient
      Caption = ' Por Cliente '
      TabOrder = 0
      object Label2: TLabel
        Left = 31
        Top = 22
        Width = 38
        Height = 14
        Caption = 'Apellido'
      end
      object Label3: TLabel
        Left = 30
        Top = 48
        Width = 37
        Height = 14
        Caption = 'Nombre'
      end
      object EApellido: TcxTextEdit
        Left = 87
        Top = 18
        Width = 121
        Height = 22
        Properties.OnChange = EApellidoPropertiesChange
        TabOrder = 0
      end
      object ENombre: TcxTextEdit
        Left = 86
        Top = 44
        Width = 121
        Height = 22
        Properties.OnChange = ENombrePropertiesChange
        TabOrder = 1
      end
    end
    object GBFecha: TGroupBox
      Left = 186
      Top = 1
      Width = 255
      Height = 78
      Align = alLeft
      Caption = ' Por Fecha '
      TabOrder = 1
      object Label1: TLabel
        Left = 23
        Top = 32
        Width = 33
        Height = 14
        Caption = 'Fecha:'
      end
      object DEFecha: TcxDateEdit
        Left = 72
        Top = 29
        Width = 145
        Height = 22
        Properties.OnChange = DEFechaPropertiesChange
        TabOrder = 0
      end
    end
    object RGFiltro: TRadioGroup
      Left = 1
      Top = 1
      Width = 185
      Height = 78
      Align = alLeft
      Caption = ' Filtro '
      ItemIndex = 0
      Items.Strings = (
        'Por Fecha'
        'Por Cliente')
      TabOrder = 2
      OnClick = RGFiltroClick
    end
  end
  object Panel3: TPanel
    Left = 0
    Top = 378
    Width = 683
    Height = 69
    Align = alBottom
    TabOrder = 3
    object Image2: TImage
      Left = 1
      Top = 1
      Width = 681
      Height = 67
      Align = alClient
      Picture.Data = {
        
        EFDD7BAF7BF75EEBDEFDD7BAFFD9}
      Stretch = True
    end
    object BSalir: TcxButton
      Left = 544
      Top = 24
      Width = 89
      Height = 25
      Caption = 'Salir'
      TabOrder = 0
      OnClick = BSalirClick
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFEFEFCFEFDFDFEFEFFFFFF}
      LookAndFeel.Kind = lfStandard
      LookAndFeel.NativeStyle = True
      UseSystemPaint = False
    end
  end
  object IBQConsulta: TIBQuery
    Database = DMGimnasio.IBDGimnasio
    Transaction = DMGimnasio.IBTGimnasio
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select distinct(fecha_ingreso),cliente from aparatos where paga=' +
        #39'S'#39)
    Left = 368
    Top = 32
    object IBQConsultaFECHA_INGRESO: TIBStringField
      FieldName = 'FECHA_INGRESO'
      Origin = 'APARATOS.FECHA_INGRESO'
      Required = True
      Size = 10
    end
    object IBQConsultaCLIENTE: TIntegerField
      FieldName = 'CLIENTE'
      Origin = 'APARATOS.CLIENTE'
      Required = True
    end
  end
  object IBQVerificarPago: TIBQuery
    Database = DMGimnasio.IBDGimnasio
    Transaction = DMGimnasio.IBTGimnasio
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select fecha_ingreso from aparatos where cliente=:id and paga='#39'S' +
        #39
      '')
    Left = 280
    Top = 33
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'id'
        ParamType = ptUnknown
      end>
    object IBQVerificarPagoFECHA_INGRESO: TIBStringField
      FieldName = 'FECHA_INGRESO'
      Origin = 'APARATOS.FECHA_INGRESO'
      Required = True
      Size = 10
    end
  end
  object IBQDatos: TIBQuery
    Database = DMGimnasio.IBDGimnasio
    Transaction = DMGimnasio.IBTGimnasio
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select nombre, apellido from clientes where id_cliente=:cliente')
    Left = 210
    Top = 34
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'cliente'
        ParamType = ptUnknown
      end>
    object IBQDatosNOMBRE: TIBStringField
      FieldName = 'NOMBRE'
      Origin = 'CLIENTES.NOMBRE'
      Size = 50
    end
    object IBQDatosAPELLIDO: TIBStringField
      FieldName = 'APELLIDO'
      Origin = 'CLIENTES.APELLIDO'
      Size = 50
    end
  end
end
