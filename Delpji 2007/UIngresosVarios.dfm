inherited FIngresosVarios: TFIngresosVarios
  Left = 406
  Top = 248
  Width = 427
  Height = 297
  Caption = 'FIngresosVarios'
  OldCreateOrder = True
  PixelsPerInch = 96
  TextHeight = 13
  inherited PEncabezado: TPanel
    Width = 419
    inherited Image1: TImage
      Width = 411
    end
    inherited LTitulo: TLabel
      Width = 125
      Caption = 'Troquelados'
    end
    inherited Image3: TImage
      Left = 256
    end
  end
  object GBTroquelados: TGroupBox
    Left = 0
    Top = 73
    Width = 419
    Height = 136
    Align = alTop
    Caption = ' Troquelados '
    Color = clWhite
    ParentColor = False
    TabOrder = 1
    object CBCombat: TcxRadioButton
      Left = 48
      Top = 40
      Width = 113
      Height = 17
      Caption = 'Body Combat'
      Checked = True
      TabOrder = 0
      TabStop = True
      LookAndFeel.Kind = lfUltraFlat
    end
    object CBPump: TcxRadioButton
      Left = 48
      Top = 72
      Width = 113
      Height = 17
      Caption = 'Body Pump'
      TabOrder = 1
    end
    object TECantidad: TcxTextEdit
      Left = 260
      Top = 70
      Width = 70
      Height = 24
      Style.Shadow = True
      TabOrder = 2
      Text = '10'
    end
    object cxLabel1: TcxLabel
      Left = 200
      Top = 73
      Width = 50
      Height = 17
      TabOrder = 3
      Caption = 'Cantidad'
    end
    object CEPresio: TcxCurrencyEdit
      Left = 260
      Top = 33
      Width = 57
      Height = 24
      EditValue = 30.000000000000000000
      Style.Shadow = True
      TabOrder = 4
    end
    object cxLabel2: TcxLabel
      Left = 208
      Top = 36
      Width = 38
      Height = 17
      TabOrder = 5
      Caption = 'Precio'
    end
  end
  object PBotones: TPanel
    Left = 0
    Top = 209
    Width = 419
    Height = 59
    Align = alClient
    TabOrder = 2
    object Image2: TImage
      Left = 1
      Top = 1
      Width = 417
      Height = 57
      Align = alClient
      Picture.Data = {
        
        14514500145145001451450014514500145145007FFFD9}
    end
    object BAceptar: TcxButton
      Left = 200
      Top = 13
      Width = 75
      Height = 25
      Caption = 'Aceptar'
      TabOrder = 0
      OnClick = BAceptarClick
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFEFEFCFEFDFDFEFEFFFFFF}
      LookAndFeel.Kind = lfStandard
      LookAndFeel.NativeStyle = True
      UseSystemPaint = False
    end
    object BCancelar: TcxButton
      Left = 304
      Top = 12
      Width = 75
      Height = 25
      Caption = 'Cancelar'
      TabOrder = 1
      OnClick = BCancelarClick
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFCFCFEFFFFFFF0F0FBF5F5FCFFFFFFFDFDFE}
      LookAndFeel.Kind = lfStandard
      LookAndFeel.NativeStyle = True
      UseSystemPaint = False
    end
  end
  object IBQTroquelados: TIBQuery
    Database = DMGimnasio.IBDGimnasio
    Transaction = DMGimnasio.IBTGimnasio
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'insert into troquelados values(:disiplina,:fecha,:cantidad,:prec' +
        'io)')
    Left = 72
    Top = 217
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'disiplina'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'fecha'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'cantidad'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'precio'
        ParamType = ptUnknown
      end>
  end
end
