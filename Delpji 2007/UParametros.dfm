inherited FParametros: TFParametros
  Left = 462
  Top = 233
  Width = 457
  Height = 330
  Caption = 'FParametros'
  OldCreateOrder = True
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  inherited PEncabezado: TPanel
    Width = 449
    inherited Image1: TImage
      Width = 441
      Stretch = True
    end
    inherited LTitulo: TLabel
      Width = 114
      Caption = 'Parametros'
    end
    inherited Image3: TImage
      Left = 296
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 73
    Width = 449
    Height = 159
    Align = alClient
    Color = clWhite
    TabOrder = 1
    object Label1: TLabel
      Left = 40
      Top = 32
      Width = 183
      Height = 13
      Caption = 'Descuento para Profesores (Aparatos) '
    end
    object Label2: TLabel
      Left = 40
      Top = 72
      Width = 188
      Height = 13
      Caption = 'Porcentaje de Aparatos para Pase Libre'
    end
    object EDescuento: TcxCurrencyEdit
      Left = 240
      Top = 28
      Width = 121
      Height = 24
      Style.Shadow = True
      TabOrder = 0
    end
    object EPorcentaje: TcxTextEdit
      Left = 243
      Top = 68
      Width = 54
      Height = 24
      Properties.MaxLength = 3
      Style.Shadow = True
      TabOrder = 1
    end
    object CBCumple: TcxCheckBox
      Left = 34
      Top = 110
      Width = 345
      Height = 21
      Properties.DisplayUnchecked = 'False'
      Properties.Caption = 'Aviso de Cumplea'#241'os en el inicio del Sistema'
      TabOrder = 2
    end
    object cxLabel1: TcxLabel
      Left = 306
      Top = 72
      Width = 16
      Height = 17
      TabOrder = 3
      Caption = '%'
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 232
    Width = 449
    Height = 69
    Align = alBottom
    TabOrder = 2
    object Image2: TImage
      Left = 1
      Top = 1
      Width = 447
      Height = 67
      Align = alClient
      Picture.Data = {
        
        14514500145145001451450014514500145145007FFFD9}
    end
    object BAceptar: TcxButton
      Left = 256
      Top = 24
      Width = 75
      Height = 25
      Caption = 'Aceptar'
      TabOrder = 0
      OnClick = BAceptarClick
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFEFEFCFEFDFDFEFEFFFFFF}
      LookAndFeel.Kind = lfStandard
      LookAndFeel.NativeStyle = True
      UseSystemPaint = False
    end
    object BCancelar: TcxButton
      Left = 352
      Top = 24
      Width = 81
      Height = 25
      Caption = 'Cancelar'
      TabOrder = 1
      OnClick = BCancelarClick
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFCFCFEFFFFFFF0F0FBF5F5FCFFFFFFFDFDFE}
      LookAndFeel.Kind = lfStandard
      LookAndFeel.NativeStyle = True
      UseSystemPaint = False
    end
  end
  object IBQGetParametros: TIBQuery
    Database = DMGimnasio.IBDGimnasio
    Transaction = DMGimnasio.IBTGimnasio
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select * from parametros')
    Left = 200
    Top = 24
  end
  object IBSPSetParametros: TIBStoredProc
    Database = DMGimnasio.IBDGimnasio
    Transaction = DMGimnasio.IBTGimnasio
    StoredProcName = 'CARGAR_PARAMETROS'
    Left = 360
    Top = 169
  end
end
