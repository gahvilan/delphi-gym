inherited FSalida: TFSalida
  Left = 297
  Top = 211
  Width = 562
  Height = 354
  Caption = 'FSalida'
  OldCreateOrder = True
  PixelsPerInch = 96
  TextHeight = 13
  inherited PEncabezado: TPanel
    Width = 554
    inherited Image1: TImage
      Width = 546
    end
    inherited LTitulo: TLabel
      Width = 149
      Caption = 'Salida de Plata'
    end
    inherited Image3: TImage
      Left = 400
    end
  end
  object PBotones: TPanel
    Left = 0
    Top = 270
    Width = 554
    Height = 55
    Align = alBottom
    TabOrder = 1
    object Image2: TImage
      Left = 1
      Top = 1
      Width = 552
      Height = 53
      Align = alClient
      Picture.Data = {
        
        14514500145145001451450014514500145145007FFFD9}
      Stretch = True
    end
    object BAceptar: TcxButton
      Left = 352
      Top = 15
      Width = 75
      Height = 25
      Caption = 'Aceptar'
      TabOrder = 0
      OnClick = BAceptarClick
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFEFEFCFEFDFDFEFEFFFFFF}
      LookAndFeel.Kind = lfStandard
      LookAndFeel.NativeStyle = True
      UseSystemPaint = False
    end
    object BCancelar: TcxButton
      Left = 439
      Top = 14
      Width = 75
      Height = 25
      Caption = 'Cancelar'
      TabOrder = 1
      OnClick = BCancelarClick
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFCFCFEFFFFFFF0F0FBF5F5FCFFFFFFFDFDFE}
      LookAndFeel.Kind = lfStandard
      LookAndFeel.NativeStyle = True
      UseSystemPaint = False
    end
  end
  object PDatos: TPanel
    Left = 0
    Top = 73
    Width = 554
    Height = 197
    Align = alClient
    Color = clWhite
    TabOrder = 2
    object CECantidad: TcxCurrencyEdit
      Left = 104
      Top = 87
      Width = 97
      Height = 24
      Style.Shadow = True
      TabOrder = 0
    end
    object cxLabel1: TcxLabel
      Left = 51
      Top = 91
      Width = 46
      Height = 17
      TabOrder = 1
      Caption = 'Salida $'
    end
    object RBVarias: TRadioButton
      Tag = 1
      Left = 56
      Top = 24
      Width = 113
      Height = 17
      Caption = 'Salidas Varias'
      Checked = True
      TabOrder = 2
      TabStop = True
    end
    object RBSalida: TRadioButton
      Tag = 2
      Left = 216
      Top = 24
      Width = 113
      Height = 17
      Caption = 'Salida de Caja'
      TabOrder = 3
    end
    object RBBebidas: TRadioButton
      Tag = 3
      Left = 368
      Top = 24
      Width = 113
      Height = 17
      Caption = 'Beer Tan'
      TabOrder = 4
    end
    object TEDescripcion: TcxTextEdit
      Left = 128
      Top = 137
      Width = 361
      Height = 24
      Style.Shadow = True
      TabOrder = 5
    end
    object cxLabel2: TcxLabel
      Left = 51
      Top = 139
      Width = 64
      Height = 17
      TabOrder = 6
      Caption = 'Descripcion'
    end
  end
  object IBQInsertSalida: TIBQuery
    Tag = 3
    Database = DMGimnasio.IBDGimnasio
    Transaction = DMGimnasio.IBTGimnasio
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'insert into salidas (cantidad,descripcion,fecha,tipo) values (:c' +
        'ant,:desc,:fecha,:tipo)')
    Left = 312
    Top = 17
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'cant'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'desc'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'fecha'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'tipo'
        ParamType = ptUnknown
      end>
  end
end
