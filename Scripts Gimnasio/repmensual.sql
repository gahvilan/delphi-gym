...
CREATE PROCEDURE ARMAR_REPORTE_MENSUAL (
    MES INTEGER,
    ANIO INTEGER,
    TIPO INTEGER)
RETURNS (
...
BEGIN
  ...
  IF (TIPO = 1) THEN
  BEGIN
       FOR SELECT SUM(CANTIDAD) FROM SALIDAS
           WHERE (fecha LIKE '%'||(CAST(:MES AS VARCHAR(2)))||'/'||(CAST (:ANIO AS VARCHAR(4)))) INTO /*:INGRESO, :DISIPLINA,*/ :SALDO
       DO
       BEGIN
            SUSPEND;
       END
