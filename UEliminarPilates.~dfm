inherited FEliminarPilates: TFEliminarPilates
  Left = 334
  Top = 169
  Height = 424
  Caption = 'FEliminarPilates'
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  inherited PEncabezado: TPanel
    inherited LTitulo: TLabel
      Width = 248
      Caption = 'Eliminar Clase de Pilates'
    end
  end
  object PBotones: TPanel
    Left = 0
    Top = 326
    Width = 687
    Height = 62
    Align = alBottom
    BevelInner = bvRaised
    TabOrder = 1
    object Image2: TImage
      Left = 2
      Top = 2
      Width = 683
      Height = 58
      Align = alClient
      Picture.Data = {
        
        14514500145145001451450014514500145145007FFFD9}
    end
    object BAceptar: TcxButton
      Left = 480
      Top = 16
      Width = 75
      Height = 25
      Caption = 'Aceptar'
      TabOrder = 0
      OnClick = BAceptarClick
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFEFEFCFEFDFDFEFEFFFFFF}
      LookAndFeel.Kind = lfStandard
      LookAndFeel.NativeStyle = True
      UseSystemPaint = False
    end
    object BCancelar: TcxButton
      Left = 576
      Top = 16
      Width = 81
      Height = 25
      Caption = 'Cancelar'
      TabOrder = 1
      OnClick = BCancelarClick
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFCFCFEFFFFFFF0F0FBF5F5FCFFFFFFFDFDFE}
      LookAndFeel.Kind = lfStandard
      LookAndFeel.NativeStyle = True
      UseSystemPaint = False
    end
  end
  object PDatos: TPanel
    Left = 0
    Top = 73
    Width = 687
    Height = 253
    Align = alClient
    Color = clWhite
    TabOrder = 2
    object LDetalleNombre: TLabel
      Left = 24
      Top = 144
      Width = 225
      Height = 13
      AutoSize = False
      WordWrap = True
    end
    object LDetalleFecha: TLabel
      Left = 24
      Top = 167
      Width = 233
      Height = 13
      AutoSize = False
      WordWrap = True
    end
    object LDetallePrecio: TLabel
      Left = 24
      Top = 191
      Width = 233
      Height = 13
      AutoSize = False
      WordWrap = True
    end
    object GroupBox1: TGroupBox
      Left = 264
      Top = 8
      Width = 409
      Height = 225
      Caption = ' Clientes '
      TabOrder = 0
      object Label1: TLabel
        Left = 24
        Top = 32
        Width = 43
        Height = 13
        Caption = 'Apellido: '
      end
      object DBGClientes: TDBGrid
        Left = 8
        Top = 64
        Width = 393
        Height = 153
        DataSource = DSClientes
        Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        ReadOnly = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        OnCellClick = DBGClientesCellClick
        Columns = <
          item
            Expanded = False
            FieldName = 'APELLIDO'
            Title.Caption = 'Apellido'
            Width = 196
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOMBRE'
            Title.Caption = 'Nombre'
            Width = 190
            Visible = True
          end>
      end
      object TEApellido: TcxTextEdit
        Left = 74
        Top = 28
        Width = 121
        Height = 24
        Properties.OnChange = TEApellidoPropertiesChange
        Style.Shadow = True
        TabOrder = 1
      end
    end
    object GroupBox2: TGroupBox
      Left = 16
      Top = 8
      Width = 233
      Height = 105
      Caption = ' Fecha de la Clase '
      TabOrder = 1
      object DEFecha: TcxDateEdit
        Left = 24
        Top = 41
        Width = 185
        Height = 24
        Properties.OnChange = DEFechaPropertiesChange
        Style.Shadow = True
        TabOrder = 0
      end
    end
  end
  object IBQClientes: TIBQuery
    Database = DMGimnasio.IBDGimnasio
    Transaction = DMGimnasio.IBTGimnasio
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'SELECT ID_CLIENTE, NOMBRE, APELLIDO FROM CLIENTES WHERE APELLIDO' +
        ' LIKE :APELLIDO ORDER BY APELLIDO, NOMBRE ASC')
    Left = 256
    Top = 337
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'APELLIDO'
        ParamType = ptUnknown
      end>
  end
  object DSClientes: TDataSource
    DataSet = IBQClientes
    Left = 312
    Top = 337
  end
  object IBQConsultaCliente: TIBQuery
    Database = DMGimnasio.IBDGimnasio
    Transaction = DMGimnasio.IBTGimnasio
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'SELECT PRECIO FROM PILATES WHERE CLIENTE = :ID_CLIENTE AND FECHA' +
        '=:FECHA')
    Left = 224
    Top = 289
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'ID_CLIENTE'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'FECHA'
        ParamType = ptUnknown
      end>
  end
  object IBQEliminar: TIBQuery
    Database = DMGimnasio.IBDGimnasio
    Transaction = DMGimnasio.IBTGimnasio
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'DELETE FROM PILATES WHERE CLIENTE = :CLIENTE AND FECHA =:FECHA')
    Left = 104
    Top = 342
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'CLIENTE'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'FECHA'
        ParamType = ptUnknown
      end>
  end
end
